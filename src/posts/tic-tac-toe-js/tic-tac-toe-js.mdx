...
![Image showing two tic tac toe boards. The first one shows the designated array index for each cell. The second shows an example of a board with a certain configuration array.](./images/config-map.png "The board on the left shows the designated array index for each cell. On the right is a board with an array configuration of: ["x","","o","o","","","x","",""].")
